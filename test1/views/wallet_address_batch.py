...
def create_wallet_address(request):
    try:
        ...
        config_json = json_loads(sitesettings.config_json)
        batch_size = 20
        ...
        for i in range(0: batch_size - unassigned_count):
            new_addresses.append(axfd_utils.create_wallet_address())
            logger.info("Create address[{0}]={1}".format(i, new_addresses[i]))

        wallet_address_query.create_user_wallets(new_addresses, 'AXFund')
